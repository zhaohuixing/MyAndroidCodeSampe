...
public class TriangleGrid extends CPuzzleGrid
{
    ...
    while (nCount < nSize) 
    {
        if(nDirection == 0)
        {
            for(int i = (nStartRow+nLeftOffset); i < nEndRow; ++i)
            {
                ...
                m_Bubbles.get(nIndex).SetLabel(nCount);
            }
            ++nRightOffset;
            ++nStartRow;
        }
...
public void InitializeGridCells()
{
    ...
    float startY = ptCenter.y - (float)(SQURT_3*fInnerSize*0.25f);
...
private void DrawMotionGridHorizontal(Canvas context)
{
    ...
    float fyOffset = (1.0f - (float)(SQURT_3*0.5f))*Math.abs(xOffset)*SHIFT_FACTOR0;
    for (int i = 0; i < m_nEdge; ++i) 
    {
        for(int j = 0; j <= i; ++j)
        {
...
private void DrawMotionGrid60Diagonal(Canvas context)
{
    ...
    PointF ptCenter;
    float fRad = m_fBubbleSize/2.0f;
    float fxOffset = (1.0f - (float)(SQURT_3*0.5f))*Math.abs(xOffset);
    for (int i = 0; i < m_nEdge; ++i) 
    {
        for(int j = 0; j <= i; ++j)
        {
...
public int GetUndoLocationInfo()
{
    ...
    {
        int nRowIndex = this.GetGridRow(m_nTouchedCellIndex);
        nRet = nRowIndex - this.GetGridColumne(m_nTouchedCellIndex);
    }
